...
const Notifications = () => {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      console.log("Notifications fetched:", res.data?.length || 0);
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (loading) {
        console.log("Loading timeout - forcing error state");
        setLoading(false);
  ...
  return (
    <div className="bg-[#F3F2EF] min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Filter Tabs - Only show if not loading */}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <button
              onClick={handleRefresh}
              className="mt-4 text-[#0A66C2] hover:underline text-sm font-medium"
            >
              Refresh
            </button>
          </div>
            ...
            {filteredNotifications.map((notification) => (
              <div
                ...
                className={`group bg-white rounded-xl shadow-sm border transition-all duration-200 hover:shadow-md ${
                  notification.read ? "border-gray-100" : "border-l-4 border-l-[#0A66C2]"
                }`}
              >
                <div className="p-5">
                        ...
                        <div className="flex items-center gap-3 mb-2">
                          <img
                            src={
                              ...
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(notification.sender.name || 'User')}&background=0A66C2&color=fff`
                            }
                            alt={notification.sender.name}
                            className="w-10 h-10 rounded-full object-cover cursor-pointer hover:ring-2 ring-[#0A66C2] transition-all"
                            onError={(e) => {
                              e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(notification.sender?.name || 'User')}&background=0A66C2&color=fff`;
                            }}
